...
export function UserDataPanel({ allProfiles, isLoading, events, onRemoveEvent }: UserDataPanelProps) {
  ...
  const handleCopyRoster = () => {
    ...
    const availablePlayers = availableUserIds
        ...
        .filter((name): name is string => !!name && allRosterPlayerNames.includes(name));

    const unavailablePlayers = allRosterPlayerNames.filter(p => !availablePlayers.includes(p));
    ...
    const availableHeader = `✅ Available Roster Players (${availablePlayers.length}):`;
    const availableList = availablePlayers.length > 0 ? availablePlayers.map(p => `- ${p}`).join('\n') : '- None';
    
    const neededText = `🔥 Players Needed: ${neededPlayers}`;
    
    const unavailableHeader = `❌ Unavailable Roster Players (${unavailablePlayers.length}):`;
  ...
  const handleCopyAllPlayers = () => {
    ...
    const playerList = allRosterPlayerNames
      .map(p => `- ${p}`)
      .join('\n');
    ...
    navigator.clipboard.writeText(fullText).then(() => {
      toast({
        title: 'Copied Roster Players',
        description: 'A list of all roster players has been copied.',
      });
    }, (err) => {
      console.error('Could not copy text: ', err);
      toast({
    ...
    <div className='space-y-8'>
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                Manage the official team roster and perform administrative actions.
                </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <ScrollArea className="border rounded-lg h-[40vh]">
                    ...
                    <TableHeader className="sticky top-0 bg-muted/80 backdrop-blur-sm">
                        ...
                        <TableHead className='text-right'>Roster Member</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
            ...
            <CardFooter className="flex-col items-start gap-4">
                ...
                <div className="w-full space-y-2">
                    <h4 className='text-sm font-medium'>Copy All Roster Player Names</h4>
                    <div className='flex items-center justify-between gap-2 p-2 border rounded-lg'>
                        <p className='text-sm text-muted-foreground'>Copy a simple list of all official roster members.</p>
                        <Button onClick={handleCopyAllPlayers} disabled={!allRosterPlayerNames || allRosterPlayerNames.length === 0}>
