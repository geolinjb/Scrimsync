...
export function HeatmapGrid({
  ...
  const handleCopyList = () => {
    ...
    const formatPlayerList = (players: SelectedSlot['players']) => {
        if (players.length === 0) return '- None';
        return players
          .map(p => {
            const profile = profileMap.get(p.name);
            const tag = p.discordUsername || p.name;
            let statusSuffix = "";
            if (profile?.rosterStatus === 'Main Roster') statusSuffix = " (Main)";
            else if (profile?.rosterStatus === 'Standby Player') statusSuffix = " (Standby)";
    ...
    const unavailableList = formatPlayerList(unavailablePlayers);
                                    ...
                                    {selectedSlot.players.map(player => (
                                        <li key={player.id} className='flex items-center gap-3 text-sm'>
                                            ...
                                            <div className='flex flex-col'>
                                                ...
                                                {player.discordUsername && <span className='text-[10px] text-muted-foreground'>{player.discordUsername}</span>}
                                            </div>
                                        </li>
                                    ))}
                                    </ul>
                                    ...
                                    {unavailablePlayers.map(player => player && (
                                        <li key={player.id} className='flex items-center gap-3 text-sm text-muted-foreground'>
                                            ...
                                            <div className='flex flex-col'>
                                                ...
                                                {player.discordUsername && <span className='text-[10px]'>{player.discordUsername}</span>}
                                            </div>
                                        </li>
                                    ))}
                                    </ul>
